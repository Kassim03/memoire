<div id="editReservationModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50 hidden">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-auto p-6">
        ...
        <form id="editReservationForm" method="POST" action="{{ route('reservation.update', ["id_reservation"=>!empty($reservation->id) ? $reservation->id : 0, "id_user"=>Auth::user()->id]) }}" class="space-y-4">
            ...
            {{-- Utilisez la directive @csrf pour inclure le token CSRF --}}
            @csrf
            @method('PUT')
            <input type="hidden" id="reservationId" name="reservation_id">
            <input type="hidden" id="emplacementId" name="emplacement_id"> {{-- Input caché pour l'ID de l'emplacement --}}
...
function openEditModalFromButton(btn) {
    ...
    const emplacementId  = btn.dataset.emplacementId;
    const emplacementNom = btn.dataset.emplacementNom;
    const dateReserv     = btn.dataset.dateReserv;
    const heureDebut     = btn.dataset.heureDebut;
    const heureFin       = btn.dataset.heureFin;
    const participants   = btn.dataset.participants;    /* action du formulaire (PUT) */
    //form.action = `/reservation/${id}`;          // adapte si tu as route('reservation.update', id)

    /* remplissage des champs */
    document.getElementById('reservationId').value   = id;
    document.getElementById('emplacementId').value   = emplacementId;
    document.getElementById('edit_salle').value      = emplacementNom;
    document.getElementById('edit_date_reserv').value= dateReserv;
    document.getElementById('participants').value = participants;
    // datetime-local doit être au format  YYYY-MM-DDTHH:MM
    document.getElementById('edit_heure_debut').value =
        heureDebut ? new Date(heureDebut).toISOString().slice(0,16) : '';
    document.getElementById('edit_heure_fin').value   =
        heureFin   ? new Date(heureFin).toISOString().slice(0,16)   : '';

    calculateDuration();        // calcule la durée immédiatement
...
function calculateDuration() {
    ...
    const diffMin = Math.floor((dFin - dDebut) / 60000);   // en minutes
...
/* déclenche le recalcul à chaque changement */
...
/* fermeture si clic hors de la boîte */
...
Remplece les donnees comme si c'etait une reservation